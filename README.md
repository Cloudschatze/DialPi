...
DialPi turns your Raspberry Pi into a dial-up server with real modem connectivity. Vintage computers can dial in via traditional phone lines to access Telnet gateway services or establish PPP connections for internet access. It's like running your own ISP from 1995.
...
-   - *Alternatively: RS-232 modem and USB-to-serial adapter (appears as `/dev/ttyUSB0` or similar - refer to the Customization section)*
...
```bash
wget https://github.com/Cloudschatze/DialPi/raw/refs/heads/main/dialpi.sh && chmod +x dialpi.sh && ./dialpi.sh
```
